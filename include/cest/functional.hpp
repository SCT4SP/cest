...
template <typename R, typename... Args> class virt_impl_f {
public:
...
public:
  ...
  constexpr function(F &&f)
      : f_(cest::make_unique<impl_f<std::decay_t<F>, R, Args...>>(
            std::move(f))) {}
};
...
public:
  ...
  template <typename F>
  constexpr function(F &&f)
      : f_(cest::make_unique<impl_f<std::decay_t<F>, void, Args...>>(
            std::move(f))) {}
};
