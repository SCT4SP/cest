...
public:
  ...
  constexpr element_type const &operator*() const noexcept { return *ptr_; }
...
template <typename T>
constexpr bool operator==(unique_ptr<T> const &a,
                          unique_ptr<T> const &b) noexcept {
  return a.get() == b.get();
}
...
template <typename T>
constexpr cest::ostream &operator<<(cest::ostream &o, unique_ptr<T> const &p) {
