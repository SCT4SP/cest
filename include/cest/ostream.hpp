...
public:
  ...
  explicit constexpr basic_ostream(__streambuf_type *__sb) { this->init(__sb); }

  virtual constexpr ~basic_ostream() {}
  ...
  constexpr __ostream_type &operator<<(__ios_type &(*__pf)(__ios_type &)) {
    __pf(*this);
    return *this;
  }

  // for hex
  constexpr __ostream_type &operator<<(ios_base &(*__pf)(ios_base &)) {
    __pf(*this);
    return *this;
  }

  constexpr __ostream_type &operator<<(long __n) { return _M_insert(__n); }
...
protected:
  constexpr basic_ostream() { this->init(0); }

  template <typename _ValueT> constexpr __ostream_type &_M_insert(_ValueT __v);
};
