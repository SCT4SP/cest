...
template <template <typename...> typename V> constexpr auto iterator_test1() {
  V<int> v;
...
template <typename It1, typename It2> constexpr bool iterator_test_helper() {
...
template <typename T1, typename T2> constexpr bool static_iterator_test() {
...
void iterator_tests() {
  ...
  static_assert(
      static_iterator_test<std::forward_list<int>, forward_list<int>>());
  ...
  using ifs_ = cest::ifstream;
  using str_ = cest::string;
